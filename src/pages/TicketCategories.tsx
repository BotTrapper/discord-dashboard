...
const TicketCategories: React.FC = () => {
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/api/ticket-categories/${guildId}?includeInactive=true`,
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        ...
        await api.put(
          `/api/ticket-categories/${guildId}/${editingCategory.id}`,
          formData,
        );
      } else {
        // Create new category
  ...
  const handleDelete = async (categoryId: number) => {
    if (
      !guildId ||
      !confirm(
        "Möchtest du diese Kategorie wirklich löschen? Alle Tickets werden von dieser Kategorie getrennt.",
      )
    ) {
      return;
    }
        ...
        {categories.length === 0 ? (
          <div className="p-6 text-center">
            <p className="text-gray-500 dark:text-gray-400">
              Noch keine Kategorien erstellt. Erstelle deine erste Kategorie, um
              zu beginnen.
            </p>
          </div>
        ) : (
