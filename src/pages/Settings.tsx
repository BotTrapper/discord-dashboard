...
export default function Settings() {
  ...
  const saveFeatureSettings = useCallback(async (featureUpdates: Record<string, boolean>) => {
    if (!guildId) return;

    try {
      setState(prev => ({ ...prev, saving: true, error: null }));

      const response = await authService.apiRequest(`/api/settings/${guildId}/features`, {
        method: 'PUT',
        headers: {
  ...
  const toggleFeature = useCallback(async (featureName: string) => {
    ...
    const isCurrentlyEnabled = state.settings.enabledFeatures.includes(featureName);
    ...
    setState(prev => ({
      ...
      settings: prev.settings ? {
        ...
        enabledFeatures: newEnabledStatus
          ...
          : prev.settings.enabledFeatures.filter(f => f !== featureName)
      } : null
    }));
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 text-lg font-medium mb-2">{state.error}</div>
          <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Server Einstellungen</h1>
              <p className="text-gray-600 dark:text-gray-300 mt-1">Verwalte die Bot-Features für diesen Server</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-medium text-gray-900 dark:text-white">Feature-Einstellungen</h2>
          ...
          <div className="p-6">
            <div className="space-y-6">
              {Object.entries(FEATURE_INFO).map(([featureKey, info]) => {
                const isEnabled = state.settings?.enabledFeatures.includes(featureKey) || false;
                const IconComponent = info.icon;

                return (
                  <div key={featureKey} className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div className={`p-3 rounded-lg ${info.bgColor} dark:bg-opacity-20`}>
                        <IconComponent className={`h-6 w-6 ${info.color} dark:text-opacity-80`} />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center">
                      ...
                      >
                        <span className="sr-only">{info.name} {isEnabled ? 'deaktivieren' : 'aktivieren'}</span>
                        <span
            ...
            {state.settings?.updatedAt && (
              <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                  ...
                  Zuletzt aktualisiert: {new Date(state.settings.updatedAt).toLocaleString('de-DE')}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                <p>
                  Das Deaktivieren von Features macht die entsprechenden Bot-Commands
                  und Funktionen sofort unverfügbar. Benutzer erhalten eine Fehlermeldung,
                  wenn sie versuchen, deaktivierte Features zu verwenden.
                </p>
              </div>
            </div>
          </div>
